...
class FeatureBIRL { // BIRL process
   ...
   public:
     ...
     FeatureBIRL(FeatureGridMDP* init_mdp, double min_reward, double max_reward, unsigned int chain_len, double step, double conf, int samp_flag=0, bool reject=false, int num_step=1, double epsilon = 0.0001):  r_min(min_reward), r_max(max_reward), step_size(step), chain_length(chain_len), alpha(conf), sampling_flag(samp_flag), mcmc_reject(reject), num_steps(num_step), eps(epsilon){ 
      ...
      double getValueIterationStoppingThreshold(){return eps;};
};
